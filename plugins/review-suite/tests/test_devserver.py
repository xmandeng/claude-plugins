"""Unit tests for the review-suite devserver.

Covers the pure logic: WebSocket framing (RFC 6455), LAN IP resolution,
the DevHandler log-filtering tweak, and the PUT-for-`*-layouts.json` request
path (resolve_safe_layouts_target + DevHandler.do_PUT). PTY/fork paths, the
HTTP server itself, and the full WS->PTY bridge are integration concerns and
...
class TestResolveLanIp:
    def test_env_override_wins(self, monkeypatch):
        monkeypatch.setenv("REVIEW_SUITE_HOST", "10.0.0.42")
        assert devserver.resolve_lan_ip() == "10.0.0.42"

    def test_uses_connected_socket_name(self, monkeypatch):
        monkeypatch.delenv("REVIEW_SUITE_HOST", raising=False)
        fake_sock = MagicMock()
