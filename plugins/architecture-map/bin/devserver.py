"""Devserver for the architecture-map plugin.

Serves the generated concept-map HTML over HTTP, bridges a browser xterm.js terminal
...
map template can persist to disk next to the HTML.

Usage:
    python3 devserver.py [port]          # default port: 8785
...
# =============================================================================
# PUT handler helpers — the map template persists named layouts via
...
def resolve_lan_ip() -> str:
    ...
    override = os.environ.get("ARCHITECTURE_MAP_HOST")
    if override:
        return override
    try:
...
def main() -> None:
    port = int(os.environ.get("ARCHITECTURE_MAP_PORT") or (sys.argv[1] if len(sys.argv) > 1 else 8785))
    ...
    print(f"architecture-map devserver: http://{lan_ip}:{port}/")
